...
- **只读**。派活、喊话两个写操作 mock 出来没意义,本版只留入口位置(禁用态 + 说明),真实现归 MTM-278。
- **数据是假的,结构是真的**。mock 严格按周构契约(`src/contract/types.ts`)生成,
  换真数据只换数据源(`?source=live` 已经能切,`/api/roster` 立刻是真的 13 人)。
